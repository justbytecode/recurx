...
export async function POST(request) {
  ...
  try {
    const { name, amount, currency, interval, type, description } = await request.json();
    if (!name || !amount || !currency || !interval || !type) {
...
export async function PUT(request) {
  ...
  try {
    ...
    const plan = await prisma.plan.update({
      where: { id: planId, merchantId: session.user.id },
      data: {
        name,
        description,
        amount: parseFloat(amount),
